...
LilyFlower::LilyFlower(std::string data) : ParameterObject() {
    ...
    upper_petal_ = TaperedPetal(split[11]);
    lower_petal_ = TaperedPetal(split[12]);
    update_matrix();
}
...
void LilyFlower::update_matrix() {
    ...
    float default_angle_upper = Vector3DotProduct(Vector3Normalize(upper_petal_.tip_vector()), Vector3{1,0,0});
    float default_angle_lower = Vector3DotProduct(Vector3Normalize(lower_petal_.tip_vector()), Vector3{1,0,0});
    ...
    float offset_lower = base_width_upper*SQRT_3/6.0f;
    ...
    Matrix inner_transform_lower = MatrixMultiply(QuaternionToMatrix(pitch_quaternion_lower),MatrixTranslate(offset_lower,0,0));
    Matrix inner_transform_lower_one = MatrixMultiply(QuaternionToMatrix(pitch_quaternion_lower),MatrixMultiply(MatrixTranslate(offset_lower,0,0),QuaternionToMatrix(splay_one)));
    Matrix inner_transform_lower_two = MatrixMultiply(QuaternionToMatrix(pitch_quaternion_lower),MatrixMultiply(MatrixTranslate(offset_lower,0,0),QuaternionToMatrix(splay_two)));

    Matrix outer_transform = MatrixMultiply(MatrixScale(scale_, scale_, scale_),MatrixMultiply(QuaternionToMatrix(quaternion_), MatrixTranslate(position_.x, position_.y, position_.z)));
    
    upper_transforms_[0] = MatrixMultiply(inner_transform_upper, outer_transform);
    upper_transforms_[1] = MatrixMultiply(inner_transform_upper_one, outer_transform);
    upper_transforms_[2] = MatrixMultiply(inner_transform_upper_two, outer_transform);

    lower_transforms_[0] = MatrixMultiply(inner_transform_lower, outer_transform);
    lower_transforms_[1] = MatrixMultiply(inner_transform_lower_one, outer_transform);
    lower_transforms_[2] = MatrixMultiply(inner_transform_lower_two, outer_transform);
}
...
void LilyFlower::initialize_parameters() {
    parameter_map_.set_parameter("PetalPitchUpper", Parameter{-90.0f,45.0f,80.0f});
    parameter_map_.set_parameter("PetalPitchLower", Parameter{-90.0f,45.0f,80.0f});
}